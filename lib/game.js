...
import {Positions, StageDivides, StartTypes} from './constants';
...
export default class Game {
    ...
    prepareStageTwo() {
        ...
        if (this.stageCount > StageDivides.TWO.beginning && this.stageCount < StageDivides.TWO.ending) {
            ...
            const newPositionOne = [...evilMinionOnePosition];
            const newPositionTwo = [...evilMinionTwoPosition];
            const newPositionThree = [...evilMinionThreePosition];
            const newPositionFour = [...evilMinionFourPosition];

            this.evilMinion.position = newPositionOne;
            this.evilMinion2.position = newPositionTwo;
            this.evilMinion3.position = newPositionThree;
            this.evilMinion4.position = newPositionFour;
    ...
    resetToStageOne() {
        ...
        if (this.stageCount > 1800) {
            ...
            this.stageCount = 0;
            this.stage = 1;
            const newPositionOne = [...skyscraperOnePosition];
            const newPositionTwo = [...skyscraperTwoPosition];
            this.obstacle.position = newPositionOne;
            this.obstacle2.position = newPositionTwo;
            this.obstacle.activated = true;
            this.obstacle2.activated = true;

            if (this.flyingObstacle) {
                const newPositionThree = [...flyingObstaclePosition];
                this.flyingObstacle.position = newPositionThree;
    ...
    drawStageOne() {
        ...
        const {HARD_START, MEDIUM_START} = StartTypes;

        if (this.difficulty === MEDIUM_START || this.difficulty === HARD_START) {
    ...
    generatePieces() {
        ...
        const {HARD_START, MEDIUM_START} = StartTypes;
        if (this.difficulty === HARD_START || this.difficulty === MEDIUM_START) {
